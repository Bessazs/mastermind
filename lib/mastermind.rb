...
module Mastermind
  class Game
    ...
    def initialize
      @list_colors = %w[Black Blue Gray Green Red Purple Violet Pink Yellow
                        White].sample(4)
      @player = Player.new
      @points_index = 0
      @points = 0
    ...
    def check_colors
      ...
      @list_colors.each_with_index do |color, index|
        @player.colors.each_with_index do |color_player, index_player|
          @points_index += 1 if color_player == color && index_player == index
          @points += 1 if color_player == color
        end
      end
      if @points_index == 4
